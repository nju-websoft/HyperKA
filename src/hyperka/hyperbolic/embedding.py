...
class Embedding(tf.keras.Model):
    ...
    def __init__(self, size, dim, manifold, sparse=True):
        ...
        self.eps = 1e-10
        scale = 1e-4
        self.emb = tf.Variable(tf.random_uniform([size, dim], -scale, scale, dtype=tf.float64), name="emb")
    ...
    def loss(self, preds, targets):
        return tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=targets, logits=preds))
