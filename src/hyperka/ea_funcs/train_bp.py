...
def bootstrapping(ref_sim_mat, ref_ent1, ref_ent2, labeled_alignment, th, top_k, is_edit=False, heuristic=True):
    n = ref_sim_mat.shape[0]
    curr_labeled_alignment = find_potential_alignment(ref_sim_mat, th, top_k, n, heuristic)
    if is_edit:
    ...
    if alignment is not None:
        entities1 = [ref_ent1[pair[0]] for pair in alignment]
        entities2 = [ref_ent2[pair[1]] for pair in alignment]
    else:
